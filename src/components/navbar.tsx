...
export default function Navbar() {
  ...
  // Detecta logout en otra pestaña
  useEffect(() => {
  ...
  return (
    <>
              ...
              return (
                <Link
                  ...
                  className={[
                    ...
                    active ? 'bg-brand-600/20 text-brand-300' : 'text-white/55 hover:bg-white/[0.07] hover:text-white',
                  ].join(' ')}
                >
                  <Icon name={icon} className="text-[17px]" filled={active} />
                  {label}
                </Link>
              )
            })}

            {/* Admin items — solo visible para admin */}
            {isAdmin && (
              <>
                ...
                {NAV_ADMIN.map(({ to, label, icon }) => {
                  ...
                  return (
                    <Link
                      ...
                      className={[
                        'flex items-center gap-1.5 rounded-lg px-3 py-2 text-[13px] font-medium transition-all duration-150',
                        active ? 'bg-violet-600/20 text-violet-300' : 'text-white/55 hover:bg-violet-500/[0.07] hover:text-violet-300',
                      ].join(' ')}
                    >
                      <Icon name={icon} className="text-[17px]" filled={active} />
                      {label}
      ...
      {/* Sidebar mobile (fuera del header para no quedar tapado) */}
